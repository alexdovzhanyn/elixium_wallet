defmodule ElixWallet.ReedSolomon do
  ...
  Example:
      iex> ElixWallet.QRCode.ReedSolomon.generator_polynomial(10)
      [0, 251, 67, 46, 61, 118, 70, 64, 94, 32, 45]
  """
  def generator_polynomial(error_code_len)

  Stream.iterate({[0, 0], 1}, fn {e, i} ->
    {rest, last} = Stream.map(e, &rem(&1 + i, 255))
      |> Enum.split(i)
    rest = Stream.zip(rest, tl(e))
      |> Enum.map(fn {x, y} ->
        ElixWallet.GaloisField.to_i(x) ^^^ ElixWallet.GaloisField.to_i(y)
        |> ElixWallet.GaloisField.to_a()
      end)
    {[0] ++ rest ++ last, i + 1}
  end)
  |> Stream.take(32)
  ...
  Example:
      iex> ElixWallet.QRCode.ReedSolomon.encode(ElixWallet.QRCode.Encode.encode("hello world!"))
  ...
  Example:
      iex> ElixWallet.QRCode.ReedSolomon.polynomial_division([64, 198, 134, 86, 198, 198, 242, 7, 118, 247, 38, 198, 66, 16, 236, 17, 236, 17, 236], [0, 87, 229, 146, 149, 238, 102, 21], 19)
  ...
  defp do_polynomial_division([h | _] = msg, gen_poly) do
    Stream.map(gen_poly, &rem(&1 + ElixWallet.GaloisField.to_a(h), 255))
    |> Enum.map(&ElixWallet.GaloisField.to_i/1)
    |> pad_zip(msg)
  ...
  def bch_encode(data) do
    bch = do_bch_encode(ElixWallet.Encode.bits(<<data::bits, 0::10>>))
    ElixWallet.Encode.bits(data) ++ bch
    |> Stream.zip(ElixWallet.Encode.bits(<<@format_mask::15>>))
    |> Enum.map(fn {a, b} -> a ^^^ b end)
  end
  ...
  defp do_bch_encode(list) do
    ElixWallet.Encode.bits(<<@format_generator_polynomial::11>>)
