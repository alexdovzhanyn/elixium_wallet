defmodule ElixWallet.Matrix do
...
(7,0)|     (7,7)
  ...
  def new(version) do
    ...
    matrix = Tuple.duplicate(nil, modules)
  ...
  def draw_seperators(%__MODULE__{matrix: matrix, modules: modules} = m) do
    z = modules - 8
    matrix = [{{0, 7}, {1, 8}}, {{0, z}, {1, 8}}, {{7, z}, {8, 1}},
       {{7, 0}, {8, 1}}, {{z, 0}, {8, 1}}, {{z, 7}, {1, 8}}]
  ...
  def draw_alignment_patterns(%__MODULE__{matrix: matrix, version: version} = m) do
    matrix = (for x <- @alignments[version], y <- @alignments[version], do: {x, y})
  ...
  def draw_data_with_mask(%__MODULE__{matrix: matrix, modules: modules} = m, data) do
    candidate = Stream.unfold(modules - 1, fn
        -1 -> nil
        8  -> {8, 5}
        n  -> {n, n - 2}
      end)
      ...
      |> Stream.filter(&available?(matrix, &1))
      |> Stream.zip(ElixWallet.Encode.bits(data))

    {mask, _, matrix} = Stream.map(0b000..0b111, fn mask ->
      matrix = Enum.reduce(candidate, matrix, fn {coordinate, v}, acc ->
        update(acc, coordinate, v ^^^ ElixWallet.Mask.mask(mask, coordinate))
      end)
      {mask, ElixWallet.Mask.score(matrix), matrix}
    end)
  ...
  def draw_data_with_mask0(%__MODULE__{matrix: matrix, modules: modules} = m, data) do
    matrix = Stream.unfold(modules - 1, fn
      ...
      |> Stream.zip(ElixWallet.Encode.bits(data))
      |> Enum.reduce(matrix, fn {coordinate, v}, acc ->
        update(acc, coordinate, v ^^^ ElixWallet.Mask.mask(0, coordinate))
      end)
  ...
  def draw_format_areas(%__MODULE__{matrix: matrix, modules: modules, mask: mask} = m) do
    data = ElixWallet.ReedSolomon.bch_encode(<<@ecc_l::2, mask::3>>)
    matrix = [{{8, 0}, {9, 1}}, {{7, 8}, {1, -6}},
       {{modules - 1, 8}, {1, -6}}, {{8, modules - 8}, {8, 1}}]
      |> Stream.flat_map(fn {a, b} -> shape(a, b) end)
  ...
  def draw_version_areas(%__MODULE__{matrix: matrix, modules: modules} = m) do
    data = ElixWallet.Encode.bits(<<0b000111110010010100::18>>)
    z = modules - 9
    matrix = [{{z, 5}, {1, -1}}, {{z, 4}, {1, -1}}, {{z, 3}, {1, -1}},
       {{z, 2}, {1, -1}}, {{z, 1}, {1, -1}}, {{z, 0}, {1, -1}},
       {{5, z}, {-1, 1}}, {{4, z}, {-1, 1}}, {{3, z}, {-1, 1}},
       {{2, z}, {-1, 1}}, {{1, z}, {-1, 1}}, {{0, z}, {-1, 1}}]
      |> Stream.flat_map(fn {a, b} -> shape(a, b) end)
      |> Stream.filter(&reserved?(matrix, &1))
      |> Stream.zip(Stream.cycle(data))
      |> Enum.reduce(matrix, fn {coordinate, v}, acc ->
  ...
  def draw_quite_zone(%__MODULE__{matrix: matrix, modules: modules} = m) do
    ...
    matrix = Enum.reduce(0..(modules - 1), matrix, fn i, acc ->
  ...
  Example:
      iex> ElixWallet.Matrix.shape({0, 0}, {3, 3})
      [{0, 0}, {0, 1}, {0, 2},
       {1, 0}, {1, 1}, {1, 2},
       {2, 0}, {2, 1}, {2, 2}]
  """
  ...
  def shape({x, y}, {w, h}) do
    for i <- x..(x + h - 1), j <- y..(y + w - 1), do: {i, j}
  end
