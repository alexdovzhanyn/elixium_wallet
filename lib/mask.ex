defmodule ElixWallet.Mask do
  ...
  """
  @spec score(ElixWallet.Matrix.matrix) :: integer
  def score(matrix) do
  ...
  """
  @spec rule1(ElixWallet.Matrix.matrix) :: integer
  def rule1(matrix) do
  ...
  """
  @spec rule2(ElixWallet.Matrix.matrix) :: integer
  def rule2(matrix) do
    z = tuple_size(matrix) - 2
    for i <- 0..z, j <- 0..z do
      ElixWallet.Matrix.shape({i, j}, {2, 2})
  ...
  """
  @spec rule3(ElixWallet.Matrix.matrix) :: integer
  def rule3(matrix) do
    z = tuple_size(matrix)
    for i <- 0..(z - 1), j <- 0..(z - 11) do
      [{{i, j}, {11, 1}}, {{j, i}, {1, 11}}]
      |> Stream.map(fn {a, b} ->
        ElixWallet.Matrix.shape(a, b)
        |> Enum.map(&get(matrix, &1))
      end)
      |> Enum.map(&do_rule3/1)
    end
  ...
  """
  @spec rule4(ElixWallet.Matrix.matrix) :: integer
  def rule4(matrix) do
    m = tuple_size(matrix)
    black = Tuple.to_list(matrix)
  ...
  The mask algorithm.
  """
  @spec mask(integer, ElixWallet.Matrix.coordinate) :: 0 | 1
